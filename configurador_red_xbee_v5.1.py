...
TIEMPO_RESPUESTA_AT_S = 1
TIEMPO_RESPUESTA_API_S = 2.0
TIEMPO_GUARDA_S = 1.100                     # La configuración por defecto de los Xbees es de GT = 1s como tiempo de espera antes y despues de +++ para entrar a modo comando
...
def entrarModoComandoAislado(puerto):
    """Entra en modo comando con AP=0 temporal y devuelve el AP original.

    El cambio temporal no se guarda con WR. Si el AP original era 1 o 2,
    se sale y se vuelve a entrar para que el módulo ya esté trabajando en
    modo transparente cuando comience la operación AT que solicitó el menú.
    El radio continúa recibiendo RF; lo que se evita es que esos datos sean
    entregados por DOUT durante la sesión de comandos.
    """
    entrarModoComando(puerto)
    tiempoFinal = time.monotonic() + TIEMPO_RESPUESTA_AT_S
    apOriginal = None

    # ATAP es la única consulta que se realiza antes del aislamiento. Si una
    # línea ajena impide interpretarla, se vuelve a consultar dentro del límite.
    ...
    if apOriginal is None:
        raise ErrorXBee(
            "no fue posible leer el AP actual antes de aislar el modo comando"
        )

    if apOriginal != 0:
        # AP=0 se aplica únicamente en memoria. ATCN aplica el cambio y permite
        # volver a entrar desde Transparent Mode; no se utiliza ATWR aquí.
