...
def iniciarModoComandoLocal(puerto):
    """Exige una entrada correcta con +++; no detecta AP ni prueba tramas API."""
    # entrarModoComando devuelve True/False. Este envoltorio transforma False
    # en un error legible para que el menú no continúe enviando comandos.
    if not entrarModoComando(puerto):
        raise ErrorXBee(
...
def consultarComando(puerto, comando):
    """Consulta un registro por texto: devuelve NI como str y los demás como int.

    El puerto debe estar abierto y el XBee debe haber aceptado +++.
    Ejemplo: ATID responde '0009' ---> devuelve el entero 9.
    Ejemplo: ATNI responde 'RASP_COORD' ---> conserva ese texto.
    """
...
def leerConfiguracion(puerto):
    ...
    for comando in COMANDOS_LECTURA_OBLIGATORIOS:
        configuracion[comando] = consultarComando(puerto, comando)
...
def leerConfiguracionLocal(puerto):
    ...
    iniciarModoComandoLocal(puerto)

    try:
        return leerConfiguracion(puerto)
    finally:
...
def mostrarConfiguracion(configuracion):
    ...
    if configuracion.get("BD") is not None:
        bd = configuracion["BD"]
        velocidad = BAUDIOS_POR_BD.get(bd)  # Ejemplo: BD=3 corresponde a 9600.
...
def pedirId(valorActual):
    ...
    while True:
        ...
        if respuesta.lower().startswith("0x"):
            respuesta = respuesta[2:]  # Acepta tanto '0x0009' como '0009'.
        ...
        if 0 <= valor <= 0x7FFF:  # Rango del firmware 900HP de este proyecto.
            return valor
...
def pedirAp(valorActual):
    """Pide el modo UART que tendrá el XBee después de salir de modo comando.

    Elegir AP=1 o AP=2 no hace que la configuración local se envíe por API.
...
def pedirNi(valorActual):
    ...
    while True:
        ...
        if any(byte < 0x20 or byte > 0x7E for byte in datos):
...
def prepararDescubrimientoApi(puerto):
    ...
    try:
        configuracion = leerConfiguracion(puerto)
        ap = configuracion["AP"]
        ...
        opcionesNo = consultarComando(puerto, "NO")
        tiempoNt = consultarComando(puerto, "NT") * 0.100
...
def operacionConfigurar(rutaPuerto, baudios):
    ...
    # El puerto ya está cerrado: el usuario puede demorarse en contestar.
